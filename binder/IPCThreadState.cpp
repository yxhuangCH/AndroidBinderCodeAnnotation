                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    if (err == NO_ERROR) {
        ...
        // 整理数据，并把结果存入 mOut 中。在 talkWithDriver() 方法中才会将命令真正发给 Binder 驱动
      ...
      mLastTransactionBinderFlags(0)
{
    ...
    clearCaller();
    // mIn 是一个 Parcel 对象，接受 Binder 发过来的数据，最大为 256
    mIn.setDataCapacity(256);
    // mOut 是一个 Parcel 对象，用于存储要发送给 Binder 的命令数据，最大容量为 256
...
status_t IPCThreadState::waitForResponse(Parcel *reply, status_t *acquireResult)
{
    ...
    int32_t err;
    while (1) {
        // talkWithDriver() 处理与 Binder 间的交互命令
